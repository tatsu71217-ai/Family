...
| 気づき | `/insights` | 記録から見えることの提示、内省の問いかけ、言い換え、行動の提案 |
...
「気づき」画面（`/insights`）に、次の4つを実装しています。実装は `src/lib/support.ts`。

1. **気づきの提示** — 記録が途絶えている人、出来事の後に重い記録が続いた時期、
   気にかかりが集まっている場面などを提示します。数え上げではなく見立てを含むものには
   必ず「推測」のしるしを付け、断定形を使いません。
2. **内省の問いかけ** — 答えを与えず、考えるための問いを返します。回答は任意で、
   いつでも飛ばせます。書き留めた回答はその人のプロフィール（Perspective）に残ります。
3. **言い換えの提案** — 相手が身構えやすい表現を指摘し、別の言い方の候補を示します。
   **元の記録は書き換えません。** 機械が文章を書き直すと日本語として壊れるため、
   置き換え文は生成せず、「私」を主語にした型（事実 → 気持ち → 望み）を
   本人が組み立てる形にしています。
4. **行動の提案** — 「小さく・自分だけで完結し・相手の変化を前提としない」ものに限定しています。
   「相手に○○させる」形の提案は生成しません。

### サポート導線への切り替え（追補 A-3）

記録の中に暴力・虐待、自傷・希死念慮、恐怖や支配関係を示す言葉を見つけた場合、
**気づき・問いかけ・行動提案の生成をすべて停止**し、相談窓口の案内だけを表示します
（`src/lib/safety.ts`）。判定は端末の中だけで行い、外部へは何も送りません。
言い換え機能の入力欄も同じ判定を通ります。

### 外部サービスを使っていないこと

上記はすべて、端末内の記録から決定的な処理で組み立てています。外部のLLMやAPIは呼び出しません。
家族の記録は非常にプライベートなため、送信しない設計を優先しました。
そのぶん、言い換えの自然さなど、生成モデルに任せたほうが質が上がる部分は残っています。
